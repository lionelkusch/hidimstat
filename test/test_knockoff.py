...
def test_knockoff_bootstrap_quantile():
    ...
    selected = modelxknockoff.selection_fdr(fdr=fdr)
...
def test_knockoff_bootstrap_e_values():
    ...
    modelxknockoff = ModelXKnockoff(n_repeat=n_repeat).fit(X)
    modelxknockoff.importance(X, y)
    selected = modelxknockoff.selection_fdr(
...
def test_invariant_with_bootstrap():
    ...
    selected_repeat = modelxknockoff_repeat.selection_fdr(fdr=fdr)
    ...
    np.testing.assert_raises(
        AssertionError,
        np.testing.assert_array_equal,
        modelxknockoff.pvalues_,
        modelxknockoff_repeat.pvalues_,
    )
    np.testing.assert_raises(
        AssertionError,
        np.testing.assert_array_equal,
        modelxknockoff.importances_,
        modelxknockoff_repeat.importances_,
    )
    np.testing.assert_array_equal(selected, selected_repeat)
...
def test_estimate_distribution():
    ...
    modelxknockoff = ModelXKnockoff(n_repeat=1, generator=geneator).fit(X)
    modelxknockoff.importance(X, y)
    selected = modelxknockoff.selection_fdr(fdr=fdr)
