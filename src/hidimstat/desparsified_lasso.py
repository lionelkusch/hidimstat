...
class DesparsifiedLasso:
    ...
    def fit(self, X, y):
        ...
        sigma_hat, beta_reid = self.memory.cache(reid, ignore=["n_jobs"])(
            X_,
            y_,
            epsilon=self.epsilon,
            tolerance=self.tolerance_reid,
            max_iterance=self.max_iteration,
        ...
        Z, precision_diagonal = self.memory.cache(
            self._compute_all_residuals, ignore=["n_jobs", "verbose"]
        )(
            X_,
            alphas,
        ...
        self.precision_diagonal = precision_diagonal
    ...
    def _check_fit(self):
        ...
        """
        if not (
            hasattr(self, "beta_hat")
            or hasattr(self, "precision_diagonal")
            or hasattr(self, "n_samples")
            or (hasattr(self, "sigma_hat") and hasattr(self, "theta_hat"))
        ):
    ...
    def importance(
        ...
        self._check_fit()
        self.epsilon = epsilon
        if hasattr(self, "sigma_hat"):
            return self._importance_single(
                confidence=confidence, distribution=distribution
            )
        elif hasattr(self, "theta_hat"):
            return self._importance_group(test=test)
        else:
            raise ValueError("The method requires to be fit before any analysis")

    def _importance_single(self, confidence=0.95, distribution="norm"):
        """
        Calculate confidence intervals and p-values for desparsified lasso estimators.
        This function computes confidence intervals for the desparsified lasso
        estimator beta_hat.
        ...
        confint_radius = np.abs(
            quantile
            * self.sigma_hat
            * np.sqrt(self.precision_diagonal)
            / np.sqrt(self.n_samples)
        )
        self.confidence_bound_max = self.beta_hat + confint_radius
        self.confidence_bound_min = self.beta_hat - confint_radius

        pvalue, pvalue_corr, one_minus_pvalue, one_minus_pvalue_corr = pval_from_cb(
            self.confidence_bound_min,
            self.confidence_bound_max,
    ...
    def _importance_group(self, test="chi2"):
        """
        ...
        """
        n_features, n_times = self.beta_hat.shape
        n_samples = self.precision_diagonal.shape[0]

        # Compute the two-sided p-values
        if test == "chi2":
            chi2_scores = (
                np.diag(multi_dot([self.beta_hat, self.theta_hat, self.beta_hat.T]))
                / self.precision_diagonal
            )
            self.two_sided_pvalue = np.minimum(
        ...
        sign_beta = np.sign(np.sum(self.beta_hat, axis=1))
        pvalue, pvalue_corr, one_minus_pvalue, one_minus_pvalue_corr = (
            pval_from_two_sided_pval_and_sign(
                self.two_sided_pvalue, sign_beta, eps=self.epsilon
            )
        )
        self.pvalue = pvalue
        return pvalue
    ...
    def _compute_all_residuals(
        self, X, alphas, gram, max_iteration=5000, tolerance=1e-3, n_jobs=1, verbose=0
    ):
        """
